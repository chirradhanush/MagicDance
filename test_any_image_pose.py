...
def visualize(args, name, batch_data, tb_writer, infer_model, global_step, nSample, nTest=1, pose_transfer=False, batch_data_2=None):
    ...
    print("c_cat_list len: ", len(c_cat_list))
    ...
    os.makedirs(os.path.join(args.local_image_dir,str(global_step),'gif'),exist_ok=True)
    del gene_img_list
    del pose_map_list
    del rec_image_list
...
def main(args):
    ...
    test_image_dataset = None
    test_image_dataloader = None
    test_image_dataloader_iter = None
    ...
    for itr in range(0, args.num_train_steps):
        # Get input
        test_batch_data = None
        os.makedirs(os.path.join(args.local_image_dir,str(itr)),exist_ok=True)
        test_batch_data_2 = None
        with torch.no_grad():
            print("args:",args)
            nSample = min(args.train_batch_size, args.val_batch_size)
            visualize(args, "val_images", test_batch_data, tb_writer, infer_model, itr, nSample=nSample, nTest=1, pose_transfer=args.pose_transfer, batch_data_2=test_batch_data_2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pose_pretrain_dir", type=str, default=None,
                        help="The resume directory where the model checkpoints will be loaded.")
    parser.add_argument("--init_path", type=str, default="/home/dchang/MagicDance/jefu/code/model_lib/ControlNet/pretrained_weights/control_sd15_ini.ckpt",
                        help="The resume directory where the model checkpoints will be loaded.")
